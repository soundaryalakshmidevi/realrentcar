                    ...
                    <div class="grid md:grid-cols-3 gap-6 mb-8 mt-16 ">
                        ...
                        <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
                            ...
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">
                                {{ $cars->where('premium_id', '!=', null)->count() }}
                                </p>
                    ...
                    <div class="mx-auto max-w-screen-xl">
                        <a href="{{ route('premium.create') }}">
                            <button
                                ...
                                <p class="mx-2  text-white  font-bold">Add Premium Car</p>
                            </button>
                            ...
                            <table class="w-full whitespace-no-wrap">
                                <thead>
                                    <tr
                                        ...
                                        <th class="px-4 py-3">Premium Start Date</th>
                                        <th class="px-4 py-3">Premium End Date</th>
                                        <th class="px-4 py-3">Premium Amount</th>
                                ...
                                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                                    @foreach ($premium as $pre)
                                        <tr class="text-gray-700 dark:text-gray-400">
                                            ...
                                            {{ $pre->premium_start_date }}
                                            </td>

                                            <td class="px-4 py-3 text-sm">
                                            {{ $pre->premium_end_date }}
                                            </td>
                                            <td class="px-4 py-3 text-sm">
                                            {{ $pre->premium_amount }}
                                            </td>
                                            ...
                                            <!-- <td class="px-4 py-3 text-sm">
                                                @if ($pre->end_date > $pre->start_date)
                                                    {{ Carbon\Carbon::parse($pre->end_date)->diffInDays(Carbon\Carbon::parse($pre->start_date)) }}
                                                    days
                                                @else
                                                    <p class="text-xl">😲</p>
                                                @endif
                                            ...
                                            <td class="flex my-4 py-3  px-6  space-x-3 ">
                                               ...
                                               <form action="{{ route('premium.destroy', ['premium' => $pre->id]) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
